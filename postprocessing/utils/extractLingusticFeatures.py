...
class FeatureExtractor:
    def __init__(
        self,
        input_data_path_trial,
        input_data_path_associations,
        output_data_path,
        ...
        self.input_df_ff = self.input_df_f.loc[
            self.input_df_f["duration"] > self.threshold
        ]

        self.input_df_ff["para_nr"] = self.input_df_ff["para_nr"].astype("category")

        self.input_df_ff["trial_num"] = self.input_df_ff.groupby(
            ["para_nr"], sort=False
        ).ngroup()
    ...
    def get_trial_num(self):
        """
        Calculate the trial number for each word and merge it into the output dataframe.
        """

        self.output_df = self.output_df.merge(
            self.input_df_ff[["para_nr", "trial_num"]].drop_duplicates(),
            on=["para_nr"],
            how="left",
        )
    ...
    def get_first_duration(self):
        ...
        input_df_f_grouped = self.input_df_ff.groupby(["para_nr", "cond_id", "word_nr"])
        ...
        first_association_df = first_association_df.rename(
            columns={"duration": "first_duration"}
        )
        ...
        self.output_df = self.output_df.merge(
            first_association_df, on=["cond_id", "para_nr", "word_nr"], how="left"
        )
    ...
    def get_total_duration(self):
        ...
        total_duration_df = total_duration_df.rename(
        ...
        self.output_df = self.output_df.merge(
            total_duration_df, on=["cond_id", "para_nr", "word_nr"], how="left"
        )
    ...
    def get_gaze_duration(self):
        ...
        # input_df_ff_grouped = self.input_df_ff.groupby(["para_nr", "cond_id"])
        ...
        for name, group in input_df_ff_grouped:
            for i in group.index:
                ...
                if (group.loc[: i - 1, "word_nr"] < group.loc[i, "word_nr"]).all():
                    gaze_duration_dict[group.loc[i, "word_nr"]] += group.loc[
                        i, "duration"
                    ]
            gaze_duration = pd.DataFrame(
                {
                    ...
                    "gaze_duration": gaze_duration_dict.values(),
                }
            ).sort_values(by="word_nr")
            gaze_duration_df = pd.concat(
                [gaze_duration_df, gaze_duration], ignore_index=True
            )
        ...
        self.output_df = self.output_df.merge(
            gaze_duration_df, on=["cond_id", "para_nr", "word_nr"], how="left"
        )
        self.output_df["gaze_duration"] = self.output_df["gaze_duration"].fillna(0)

    # first-pass reading time (eye-tracking "gaze duration"): sum of the durations of the consecutive
    # associations on a word starting from its first-pass entry (the first association on the word,
    # provided no word to its right has been visited yet) up to the first association on any other word.
    # Unlike get_gaze_duration it also counts directly following associations on the same word, which
    # exist when a -1 (no word) or sub-threshold association between them was removed.
    # 0 if the word was not fixated in the first-pass
    def get_first_pass_duration(self):
        fpd_dict = defaultdict(int)
        input_df_ff_grouped = self.input_df_ff.groupby(["cond_id", "para_nr"])
        fpd_df = pd.DataFrame()
        for name, group in input_df_ff_grouped:
            for i in group.index:
                if (group.loc[: i - 1, "word_nr"] < group.loc[i, "word_nr"]).all():
                    fpd_dict[group.loc[i, "word_nr"]] += group.loc[i, "duration"]
                    for index in group.loc[i + 1 :, "index"]:
                        if group.loc[index, "word_nr"] == group.loc[i, "word_nr"]:
                            fpd_dict[group.loc[i, "word_nr"]] += group.loc[
                                index, "duration"
                            ]
                        else:
                            break
            fpd = pd.DataFrame(
                {
                    "cond_id": name[0],
                    "para_nr": name[1],
                    "word_nr": fpd_dict.keys(),
                    "first_pass_duration": fpd_dict.values(),
                }
            ).sort_values(by="word_nr")
            fpd_df = pd.concat([fpd_df, fpd], ignore_index=True)
            fpd_dict.clear()
        self.output_df = self.output_df.merge(
            fpd_df, on=["cond_id", "para_nr", "word_nr"], how="left"
        )
        self.output_df["first_pass_duration"] = self.output_df[
            "first_pass_duration"
        ].fillna(0)
    ...
    def get_right_bounded_rt(self):
            ...
            rrt = pd.DataFrame(
                {
                    ...
                    "right_bounded_rt": rrt_dict.values(),
                }
            ).sort_values(by="word_nr")
        ...
        self.output_df = self.output_df.merge(
            rrt_df, on=["cond_id", "para_nr", "word_nr"], how="left"
        )
        self.output_df["right_bounded_rt"] = self.output_df["right_bounded_rt"].fillna(
            0
        )
    ...
    def get_go_past_time(self):
        ...
        for name, group in input_df_ff_grouped:
            for i in group.index:
                if (group.loc[: i - 1, "word_nr"] < group.loc[i, "word_nr"]).all():
                    ...
                    for index in group.loc[i + 1 :, "index"]:
                        if group.loc[index, "word_nr"] <= group.loc[i, "word_nr"]:
                            gpt_dict[group.loc[i, "word_nr"]] += group.loc[
                                index, "duration"
                            ]
                        else:
                            break
        ...
        self.output_df = self.output_df.merge(
            gpt_df, on=["cond_id", "para_nr", "word_nr"], how="left"
        )
    ...
    def check_comprehension_answer(self):
        ...
        input_df_ff_grouped = self.input_df_ff.groupby(
            ["cond_id", "para_nr", "word_nr"]
        )
        ...
        # self.output_df = self.output_df.drop(columns="response_chosen").merge(
        #     solution_to_fill_df, on=["cond_id", "para_nr"], how="left"
        # )
    ...
    def get_binary(self):
        ...
        for name, group in input_df_ff_grouped:
            ...
            RegIn_incl = pd.DataFrame(
                {
                    ...
                    "RegIn_incl": RegIn_incl_dict.values(),
                }
            ).sort_values(by="word_nr")
            RegIn_excl = pd.DataFrame(
                {
                    ...
                    "RegIn_excl": RegIn_excl_dict.values(),
                }
            ).sort_values(by="word_nr")
            ...
            FPFix = FPFix.merge(
                RegIn_excl, on=["cond_id", "para_nr", "word_nr"], how="left"
            )
            FPFix = FPFix.merge(
                RegIn_incl, on=["cond_id", "para_nr", "word_nr"], how="outer"
            )
        ...
        self.output_df = self.output_df.merge(
            binary_df, on=["cond_id", "para_nr", "word_nr"], how="left"
        )
        ...
        self.output_df["RegIn_incl"] = (
            self.output_df["RegIn_incl"].fillna(0).astype(int)
        )
        self.output_df["RegIn_excl"] = (
            self.output_df["RegIn_excl"].fillna(0).astype(int)
        )
    ...
    def write_out(self):
        ...
        # self.output_df.loc[
        #     self.output_df["response_chosen"] == "--", "correctness":"FPReg"
        # ] = np.nan
        # self.output_df = self.output_df[~self.output_df.response_chosen.isna()]

        # filtered_df = self.output_df[self.output_df["response_chosen"] != "--"]
        # filtered_df = self.output_df
        # Keep only the trials this participant actually read (at least one association);
        # the trial file lists every item of every list. A trial without an answer is
        # kept, with response_chosen = NaN.
        read_trials = set(self.input_df_ff["para_nr"].astype(str))
        filtered_df = self.output_df[
            self.output_df["para_nr"].astype(str).isin(read_trials)
        ]
        filtered_df.to_csv(
            self.output_path / f"reader_{self.output_name_stem}_reading_measures.csv",
            index=False,
        )
